...
def _t() -> str:
    return _kst_now().strftime("[%H:%M:%S]")

def _safe_mkdir(p: Path):
    """If 'p' exists as a file, move it aside; if dir, keep; else create."""
    if p.exists():
        if p.is_file():
            bak = p.with_suffix(p.suffix + f".old-{_kst_now().strftime('%Y%m%d%H%M%S')}")
            p.rename(bak)
        elif p.is_dir():
            return
        else:
            # unknown type -> remove then create
            try: p.unlink()
            except Exception: pass
    p.mkdir(parents=True, exist_ok=True)

def init_logging():
    global RUN_LOG, LATEST, WRITTEN
    _safe_mkdir(LOG_DIR)
    RUN_LOG = LOG_DIR / f"run-{_kst_now().strftime('%Y%m%dT%H%M%S%z')}.log"
...
def log(msg: str):
    line = f"{_t()} {msg}"
    print(line)
    if RUN_LOG: _write_file(RUN_LOG, line)
    if LATEST:  _write_file(LATEST, line)
...
def log_error(msg: str, e: Optional[BaseException]=None):
    ...
    if e:
        ...
        if RUN_LOG: _write_file(RUN_LOG, tb)
        if LATEST:  _write_file(LATEST, tb)

def note_written(s: str):
    if WRITTEN: _write_file(WRITTEN, s.rstrip())
...
# 원본 → 정식명
CANON_REG = {
    "서울특별시":"서울특별자치시" if False else "서울특별시",  # 서울 명칭 고정
    "세종특별자치시":"세종특별자치시",
    "강원도":"강원특별자치도",
    "강원특별자치도":"강원특별자치도",
    "경기도":"경기도",
...
def agg_prices(df: pd.DataFrame) -> Dict[str, Dict[str,float]]:
    ...
    # 전국
    put("전국", price_eok.index==price_eok.index)
    ...
    # 압구정동
    put("압구정동", is_seoul & (df["구"].map(canon_gu)==norm("강남구")) & (df["법정동"].map(norm)==norm("압구정동")))
...
def write_month_summary(ws: gspread.Worksheet,
    ...
    for head, key in SUMMARY_COL_TO_KEY.items():
        ...
        ws.update([[int(counts.get(key, 0))]], a1(row, cidx))
    ...
    for head, key in SUMMARY_COL_TO_KEY.items():
        cidx = col_for(head)
        if not cidx: continue
        ws.update([[float(prices.get(key, {}).get("median", 0.0))]], a1(row2, cidx))
    ...
    for head, key in SUMMARY_COL_TO_KEY.items():
        cidx = col_for(head)
        if not cidx: continue
        ws.update([[float(prices.get(key, {}).get("mean", 0.0))]], a1(row3, cidx))
    ...
    for head, key in SUMMARY_COL_TO_KEY.items():
        ...
        cell = ws.range(a1(row4, cidx))[0]
        ws.update([[display]], a1(cell.row, cell.col))
        if diff != 0:
            r,g,b = (0.0,0.35,1.0) if diff>0 else (1.0,0.2,0.2)
            color_reqs.append({
              "repeatCell":{
                "range": {
                  "sheetId": sid,
...
def build_row_by_header(header: List[str], day: date, series_norm: Dict[str,int]) -> List:
    ...
    for i, h in enumerate(header):
        if i==0:
            row.append(f"{day.year}. {day.month}. {day.day}"); continue
        nh = norm(h)
...
def main():
    ...
    for x in nat_files:
        ...
        if ws_se:
            mode, row = upsert_row(ws_se, today_kst, counts)
