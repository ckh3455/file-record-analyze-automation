...
# 광역 표기를 요약 열로 매핑 (월별 탭은 헤더가 원본과 일치한다고 하셨으니 매핑은 요약 전용)
PROV_MAP = {
    "서울특별시":"서울",
    "세종특별자치시":"세종시",
    "강원특별자치도":"강원도",
...
WHERE_WRITTEN = LOG_DIR / "where_written.txt"
...
def note_where(s: str):
    try:
        with WHERE_WRITTEN.open("a", encoding="utf-8") as f:
            f.write(s.rstrip()+"\n")
    except Exception:
        pass
...
def log_kv_list(title: str, items, limit: int = 30):
    try:
        pairs = list(items)
    except Exception:
        pairs = list(items.items()) if isinstance(items, dict) else []
    head = pairs[:limit]
    tail = pairs[limit:]
    log(f"[DEBUG] {title} ({len(pairs)} items) head={head}{' ... +%d more'%len(tail) if tail else ''}")
...
def agg_all_stats(df: pd.DataFrame):
    ...
    원본 기준으로 집계:
    - 전국: 전체 건수/중앙값/평균가
    - 서울: 광역=='서울특별시' 필터 후 건수/중앙값/평균가
    - 자치구: 서울 필터 후 구별 집계 (헤더명 그대로)
    - 압구정동: 서울 필터 후 법정동=='압구정동'
    - 광역 집계(요약용): PROV_MAP으로 요약명에 맞춰서도 집계
    """
...
def write_month_sheet_simple(ws: gspread.Worksheet, date_label: str, counts: Dict[str,int], scope: str):
    """
    ✅ 월별 탭 간단/정확 기록 + 초강력 디버깅
    - A열 날짜는 무조건 기록
    - '총합계'는 scope에 맞춰 counts['전국'] 또는 counts['서울']
    - 그 외 헤더는 counts에 존재할 때만 기록
    - 무엇을/어디에/무슨 값으로 썼는지 로그
    - 쓰기 직후 read-back으로 검증 로그
    """
    # 요약 탭에는 쓰지 않음
    ...
    if DEBUG:
        log(f"[DEBUG] >>> Writing monthly sheet: title='{ws.title}', row={row_idx}, scope={scope}")
        log(f"[DEBUG] header(#{len(header)}): {header}")
        # PREVIEW: top of counts
        log_kv_list("counts.keys", sorted(counts.items(), key=lambda x: x[0]), 60)
    ...
    for h in header:
        ...
        if h in ("총합계","합계"):
            ...
            if h not in counts:
                if DEBUG:
                    log(f"[DEBUG]   skip header='{h}' (not in counts)")
                continue
            try:
        ...
        c = hmap.get(h)
        if c:
            rng = f"{a1_col(c)}{row_idx}"
            to_write.append((rng, h, val))

    if DEBUG:
        log(f"[DEBUG] prepared write items: {[(r,h,v) for r,h,v in to_write]}")

    for rng, h, val in to_write:
        payload.append({"range": rng, "values": [[val]]})

    if payload:
        values_batch_update(ws, payload)
        log(f"[ws] {ws.title} -> {date_label} row={row_idx} (wrote {len(payload)} cells incl. date)")

        # 쓰고 나서 해당 행 다시 읽어서 검증
        try:
            written_row = _retry(ws.row_values, row_idx)
            if DEBUG:
                row_map = {}
                for i, h in enumerate(header, start=1):
                    if not h:
                        continue
                    val = written_row[i-1] if i-1 < len(written_row) else ""
                    row_map[h] = val
                log_kv_list(f"row#{row_idx} read-back", list(row_map.items()), 80)
        except Exception as e:
            log(f"[DEBUG][read-back] failed: {e!r}")

        note_where(f"{ws.title}\t(id={ws.id})\tA{row_idx}:{a1_col(len(header))}{row_idx}")
    else:
        log(f"[WARN] nothing to write for {ws.title} (ensuring date only)")
        values_batch_update(ws, [{"range": f"A{row_idx}", "values": [[date_label]]}])
...
def ym_from_filename(fname: str):
    ...
    return f"전국 2025년 {mm}월" if yy=="25" else f"전국 20{yy}년 {mm}월", \
           f"서울 2025년 {mm}월" if yy=="25" else f"서울 20{yy}년 {mm}월", \
           f"{yy}/{mm}"
...
def main():
    ...
    try:
        ...
        WHERE_WRITTEN.write_text("", encoding="utf-8")
    except Exception:
        pass
    ...
    for p in files:
        ...
        # 월별 탭 기록 (있는 탭에만; 알려진 헤더만 기록)
        ws_nat = fuzzy_ws(sh, nat_title)
        if ws_nat:
            write_month_sheet_simple(ws_nat, today_label, counts, scope="전국")

        ws_se = fuzzy_ws(sh, se_title)
        if ws_se:
            write_month_sheet_simple(ws_se, today_label, counts, scope="서울")
