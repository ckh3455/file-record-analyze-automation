...
def color_cells_batch(ws: gspread.Worksheet, row: int, col_rgb_list: List[Tuple[int, Tuple[float,float,float]]]):
    """여러 셀 색칠을 한 번의 batchUpdate로 처리 (HTTP 1회)."""
    if not col_rgb_list:
        return
    reqs = []
...
def write_counts_to_sheet(ws: gspread.Worksheet, write_date: datetime, header_names: list[str], series_map: dict, sum_col_name: str = "총합계"):
    ...
    for name in header_names:
        if name in hmap:
            ...
            if name != "전국":  # 총합계는 모든 열 합계(전국 포함/비포함은 시트 구성에 따라 조정)
                total += v
    # "전국" 값을 총합계로 쓰고 싶으면 위의 if 제거하고 total=series_map["전국"] 로 바꾸면 됨.

    if sum_col_name in hmap:
        payload.append({"range": f"{a1_col(hmap[sum_col_name])}{row}", "values": [[series_map.get('전국', total)]]})
...
def parse_file_meta(p: Path):
    ...
    name = p.stem
    m = re.search(r"\s(\d{2})(\d{2})_", name)
...
def write_month_summary(ws: gspread.Worksheet, y: int, m: int, counts: dict, med: dict, mean: dict, prev_counts: Optional[dict]):
    ...
    for k,v in diff_line.items():
        if k in header_map and v not in ("", "0"):
            col = header_map[k]
            if v.startswith("+"):
                color_jobs.append((col, (0.0, 0.35, 1.0)))  # 파랑
            elif v.startswith("-"):
                color_jobs.append((col, (1.0, 0.0, 0.0)))   # 빨강
    if color_jobs:
        color_cells_batch(ws, r4, color_jobs)
...
def ensure_apgu_sheet(sh: gspread.Spreadsheet) -> gspread.Worksheet:
    ...
    if ws:
        return ws
    return _retry(sh.add_worksheet, title=APGU_SHEET, rows=3000, cols=80)
...
def append_change_log(ws: gspread.Worksheet, added_rows: list[list], removed_rows: list[list], header: list[str]):
    ...
    if needed_rows > ws.row_count:
        _retry(ws.add_rows, needed_rows - ws.row_count)
